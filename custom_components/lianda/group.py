...
from homeassistant.components.cover import (
    DOMAIN, PLATFORM_SCHEMA, CoverEntity, ATTR_POSITION,
...
CONF_GROUP_POSITION = 'group_position'

POS_CALC_MINIMUM = 'minimum'
POS_CALC_MAXIMUM = 'maximum'
POS_CALC_AVERAGE = 'average'
POS_CALC_DEFAULT = 'default'    # checks all positions are equal or else sets
#                                 assumed_state = True

# CALC_POSITION_MAP = {
#    'default': POS_CALC_DEFAULT,
#    'minimum': POS_CALC_MINIMUM,
#    'maximum': POS_CALC_MAXIMUM,
#    'average': POS_CALC_AVERAGE
# }

PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    vol.Optional(CONF_NAME, default=DEFAULT_NAME): cv.string,
    vol.Required(CONF_ENTITIES): cv.entities_domain(DOMAIN),
    vol.Optional(CONF_GROUP_POSITION, default=POS_CALC_AVERAGE): cv.string,
})
...
async def async_setup_platform(hass, config, async_add_entities,
    ...
    async_add_entities(
        [CoverGroup(config[CONF_NAME], config[CONF_ENTITIES],
                    config[CONF_GROUP_POSITION])])


class CoverGroup(CoverEntity):
    """Representation of a CoverGroup."""

    def __init__(self, name, entities, group_position):
        ...
        _LOGGER.debug("custom cover.group {} init. group_position;{}".
                      format(name, group_position))
    ...
    async def async_update(self):
        ...
        if self._covers[KEY_POSITION]:
            ...
            if self._group_position == POS_CALC_DEFAULT:
            ...
            for entity_id in self._covers[KEY_POSITION]:
                ...
                if position == -1:
                    ...
                    if self._group_position == POS_CALC_MINIMUM:
                        position = min(pos, position)
                    elif self._group_position == POS_CALC_MAXIMUM:
                        position = max(pos, position)
                    elif self._group_position == POS_CALC_AVERAGE:
                        position += pos
                        npos += 1
                    else:   # POS_CALC_DEFAULT
                        if position != pos:
                            self._assumed_state = True
                            break
            else:
                if position != -1:
                    if self._group_position == POS_CALC_AVERAGE:
                        self._cover_position = position // npos
                    else:
